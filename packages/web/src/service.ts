...
/**
 * 管理员：列出指定群组的全部消息（包含已撤回 deleted=true）
 */
export async function adminGetGroupMessages(
    ...
    const [err, res] = await fetch('adminGetGroupMessages', {
        groupId,
        page,
        pageSize,
    });
...
/**
 * 管理员：列出指定群组的全部成员（数据库成员 + creator）
 */
export async function adminGetGroupUsers(groupId: string) {
    const [err, res] = await fetch('adminGetGroupUsers', { groupId });
    if (err) return null;
    return res;
}
...
export async function hardDeleteMessage(messageId: string) {
    const [err] = await fetch('hardDeleteMessage', { messageId });
    return !err;
}
...
export async function hardDeleteUser(userId: string) {
    const [err] = await fetch('hardDeleteUser', { userId });
    return !err;
}
